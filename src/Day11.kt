...
fun main() {
    ...
    val (_, _, panels) = doSteps(initialComputerState, initialRobot, emptyList())
    val answer = panels.size
    render(panels)
    println(answer)
}
...
fun doNextStep(
    ...
    val currentPanel = panels.firstOrNull { panel -> panel.x == robot.x && panel.y == robot.y } ?: Panel(robot.x, robot.y)
...
tailrec fun doSteps(
    ...
    isAwaitingInput(computerState) -> {
        val (updatedComputer, updatedRobot, updatedPanels) = doNextStep(computerState, robot, panels)
        doSteps(updatedComputer, updatedRobot, updatedPanels)
    }
