...
fun main() {
    ...
    val answer = distanceToClosestIntersection(input1.toPath(), input2.toPath()) // takes a few seconds to compute!
    println(answer)
}
...
fun traverse(fullPath: Path): List<Point> {

    tailrec fun loop(path: Path, acc: List<Point>): List<Point> {
...
fun intersections(p1: List<Point>, p2: List<Point>): Set<Point> = p1.toSet() intersect p2.toSet() - centralPort

fun distanceToClosestIntersection(path1: Path, path2: Path): Int? =
    intersections(traverse(path1), traverse(path2)).map { it.distanceToCentralPort() }.min()
