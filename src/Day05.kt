...
fun main() {
    ...
    val answer = doOps(0, initialMemory, 1, emptyList()).output
    println(answer)
}
...
data class Operation(val opCode: Int, val paramMode0: Mode, val paramMode1: Mode, val paramMode2: Mode)
...
fun parseOp(x: Int): Operation? {
    ...
    return if (opCodeRegex.matches(opString)) {
        Operation(
            ...
            paramMode0 = if (opString[2] == '0') Position else Immediate,
            paramMode1 = if (opString[1] == '0') Position else Immediate,
            paramMode2 = if (opString[0] == '0') Position else Immediate
        )
    } else null
}

data class Result(val memory: Memory, val output: List<Int>)
...
fun doOps(instructionPointer: Int, memory: Memory, input: Int, accumulatedOutput: List<Int>): Result {
    ...
    fun nextInstruction(): Instruction? {
        return parseOp(memory.address(instructionPointer))?.let { op ->
            when (op.opCode) {
                1, 2 -> Instruction(op, (1..3).map { memory.address(instructionPointer + it) })
    ...
    fun processInstruction(instruction: Instruction): Result {
        ...
        fun read(paramIndex: Int, mode: Mode): Int = with(instruction) {
            when (mode) {
        ...
        fun binaryInstruction(f: (Int, Int) -> Int): Memory = with(instruction) {
            val result = f(read(0, operation.paramMode0), read(1, operation.paramMode1))
        ...
        return when (instruction.operation.opCode) {
            ...
            4 -> {
                val read = when (instruction.operation.paramMode0) {
                    Position -> memory.address(instruction.parameters[0])
                    Immediate -> instruction.parameters[0]
                }
                Result(memory, accumulatedOutput + read)
            }
